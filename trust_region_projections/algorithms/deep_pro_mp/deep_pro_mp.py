...
from trust_region_projections.mps.pro_mp import DeterministicProMP
...
from trust_region_projections.sampling.mp_sampler import MPSampler
...
from trust_region_projections.utils.torch_utils import generate_minibatches, get_numpy, select_batch, \
    tensorize
...
class DeepProMP(AbstractAlgorithm):
    def __init__(self,
                 sampler: MPSampler,
                 policy: ProMPWrapper,
                 critic: BaseCritic,
        ...
        self.pad = ch.nn.ConstantPad1d((1, 0), True)
    ...
    def setup_stores(self):
        ...
        """
        # Logging setup
        super(DeepProMP, self).setup_stores()
    ...
    def advantage_and_return(self, rewards: ch.Tensor, values: ch.Tensor, dones: ch.Tensor,
        ...
        if self.use_gae:
            gae = 0
            # advantages = ch.zeros_like(values)
            # td = rewards + pcont * values[1:] - values[:-1]
            # for step in reversed(range(rewards.size(0))):
            #     # td = rewards + pcont * values[step + 1] - values[step]
            #     advantages[step] = (td + pcont[step] * self.gae_scaling * advantages[step + 1]) * time_limit_masks[step]
    ...
    def update_policy(self, dataset: TrajectoryOnPolicy):
        ...
        if self.projection.initial_entropy is None:
            self.projection.initial_entropy = self.policy.entropy(dataset.q).mean()

        batch_size = dataset.obs.shape[0]
        # mini_batches = self.n_minibatches
        mini_batches = 1

        for _ in range(self.epochs):
            # batch_indices = generate_minibatches(obs.shape[0], self.n_minibatches)

            # first observations of each trajectory
            # done flag shifted by one + first observation in array
            first_idx = self.pad(dataset.dones[:-1])
            first_obs = dataset.obs[first_idx]
            # first_idx.nonzero().flatten().diff()

            for _ in range(mini_batches):
                des_pos, des_vel, std = self.policy(first_obs)
                # (des_pos - cur_pos)
                # mean = self.policy.act(des_pos, des_vel, dataset.cur_pos, None)
                p_flat = des_pos.flatten(0, 1), std.flatten(0, 1)

                # Unlike step-based learning sample minibatch every time, because we need to predict the whole trajectory
                indices = np.random.choice(p_flat[0].shape[0], (batch_size,), replace=False)
                batch = select_batch(indices, dataset.actions, dataset.logpacs, dataset.advantages, dataset.q[0],
                                     dataset.q[1], p_flat[0], p_flat[1])
                b_actions, b_old_logpacs, b_advantages, b_old_mean, b_old_std, b_mean, b_std = batch
                b_q = (b_old_mean, b_old_std)
                b_p = (b_mean, b_std)

                # p = (des_pos, std)
                proj_p = self.projection(self.policy, b_p, b_q, self._global_steps)
                ...
                # Trust region loss
                trust_region_loss = self.projection.get_trust_region_loss(self.policy, b_p, proj_p)
                ...
                if self.vf_coeff > 0 and not self.critic:
                    ...
                    batch_vf = select_batch(indices, dataset.obs, dataset.returns, dataset.values)
                    vs = self.policy.get_value(batch_vf[0])
                    critic_loss = self.value_loss(vs, batch_vf[1], batch_vf[2])  # b_returns, b_old_values)
                    loss += self.vf_coeff * critic_loss
        ...
        steps = self.epochs * mini_batches
    ...
    def update_critic(self, dataset: TrajectoryOnPolicy):
        ...
        if self.verbose >= 1:
            stats_dict.update({k: v / steps for k, v in stats_dict.items()})
            stats_dict = collections.OrderedDict(returns=returns.mean(), old_values=old_values.mean(),
                                                 advantages=dataset.advantages.mean(), **stats_dict)
    ...
    def sample(self) -> TrajectoryOnPolicy:
        ...
        """
        with ch.no_grad():
            data = self.sampler.run(self.n_training_samples, self.policy, self.critic)
            # (obs, actions, logpacs, rewards, values, dones, time_limit_dones, terminal_obs, old_means,
            #  old_stds) = datar
            ...
            if self.max_entropy_coeff > 0:
                ...
                rewards += self.max_entropy_coeff * self.policy.entropy((data.means, data.stds)).detach()

            # Calculate advantages and returns
            advantages, returns = self.advantage_and_return(rewards, data.values, data.dones, data.time_limit_dones)

        # Unrolled sampling (T, n_envs, ...) -> (T*n_envs, ...) to train in one forward pass
        # unrolled = map(flatten_batch, (
        # data.obs, data.actions, data.logpacs, rewards, returns, advantages, data.values[:-1], data.dones,
        # data.time_limit_dones))
        # q_unrolled = tuple(map(flatten_batch, (data.means, data.stds)))
        ...
        return TrajectoryOnPolicy(
            data.obs, data.actions, data.logpacs, rewards, returns, advantages, data.values[:-1], data.dones,
            data.time_limit_dones, data.terminal_obs, (data.means, data.stds))
    ...
    def step(self):
        ...
        loss_dict_critic = stats_dict_critic = {}
        ...
        loss_dict, stats_dict = self.update_policy(dataset)

        assert loss_dict.keys().isdisjoint(loss_dict_critic.keys())
        loss_dict.update(loss_dict_critic)

        if self.verbose >= 1:
            assert stats_dict.keys().isdisjoint(stats_dict_critic.keys())
        ...
        logging_step = self._global_steps * self.n_training_samples
        ...
        metrics_dict = self.log_metrics(dataset.obs[self.pad(dataset.dones[:-1])], dataset.q, logging_step)
    ...
    def log_metrics(self, obs, q, logging_step):
        ...
        """
        with ch.no_grad():
            des_pos, des_vel, std = self.policy(obs)
            p = des_pos.flatten(0, 1), std.flatten(0, 1)
    ...
    def agent_from_data(store, train_steps=None, checkpoint_iteration=-1):
        ...
        agent = DeepProMP.agent_from_params(agent_params, store)
    ...
    def agent_from_params(params, store=None):
        ...
        policy = get_policy_network(proj_type=params['projection']['proj_type'], squash=False, device=device,
                                    dtype=dtype, obs_dim=obs_dim, action_dim=action_dim * params['pro_mp']['n_basis'],
                                    vf_model=critic if params['algorithm']["critic_coeff"] != 0 else None,
                                    **params['policy'])

        pro_mp = DeterministicProMP(**params['pro_mp'], action_dim=action_dim, dt=env.dt)
        policy = ProMPWrapper(policy, pro_mp)

        # environments
        sampler = MPSampler(discount_factor=params['algorithm']['discount_factor'], scale_actions=False,
                            disable_timelimit=False, cpu=use_cpu, dtype=dtype, seed=seed, **params['environment'])
