...
class SAC(AbstractAlgorithm):
    ...
    def get_samples_and_logpacs(self, obs: ch.Tensor, return_distributions: bool = False):
        """
        ...
        proj_p = p
        new_actions = self.policy.rsample(proj_p)
    ...
    def update_policy_and_alpha(self, obs: ch.Tensor):
        """
        ...
        squashed_actions, logpacs, p, proj_p = self.get_samples_and_logpacs(obs, return_distributions=True)

        # print("Actions Actor, std {} - mean  {}".format(*ch.std_mean(squashed_actions)))

        loss_dict = self._update_alpha(logpacs)

        q_new_actions = self.critic((obs, squashed_actions))
        alpha = self.log_alpha.exp()
        policy_loss = (alpha * logpacs - q_new_actions).mean()
        # policy_loss_unscaled = (logpacs - q_new_actions).mean()  # just for logging
        ...
        #     polyak_update(self.policy, self.old_policy, self.polyak_weight)
    ...
    def epoch_step(self):
        ...
        for _ in range(self.updates_per_epoch):
            ...
            # ############################################################################################################
            # # generate n new samples per iteration based on new policy
            if self._epoch_steps % self.sample_frequency == 0:
                self.replay_buffer.add_samples(self.sample(self.n_samples))  # , reset_envs=self._epoch_steps == 1))
            ...
            loss_dict_policy, stats_dict_policy = self.update_policy_and_alpha(batch.obs)
    ...
    def step(self):
        ...
        if self.log_interval != 0 and self._global_steps % self.log_interval == 0:
            logging_step = self._global_steps * self.n_training_samples_per_epoch + self.initial_samples
    ...
    def learn(self):
        ...
        if self.initial_samples > 0:
            samples = self.sample(self.initial_samples, random_actions=True)
            self.replay_buffer.add_samples(samples)
            # print("Actions Buffer Initial, std {} - mean  {}".format(*ch.std_mean(samples.actions)))
    ...
    def agent_from_data(store, train_steps=None, checkpoint_iteration=-1):
        ...
        agent = SAC.agent_from_params(agent_params, store)
