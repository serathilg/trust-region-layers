...
from trust_region_projections.utils.projection_utils import gaussian_wasserstein_non_commutative
from trust_region_projections.utils.torch_utils import sqrtm_newton
...
class WassersteinProjectionLayerNonCommuting(BaseProjectionLayer):
    ...
    def _trust_region_projection(self, policy: AbstractGaussianPolicy, p: Tuple[ch.Tensor, ch.Tensor],
        ...
        mean_part, cov_part, eigvals, eigvecs = gaussian_wasserstein_non_commutative(policy, p, q, self.scale_prec,
                                                                                     return_eig=True)
        ...
        prod_inv[mask] = eigvecs[mask] @ (1 / eigvals[mask].sqrt()).diag_embed(0, -2, -1) @ \
